  ...
  imports: [
    ...
    MongooseModule.forRootAsync({
      ...
      useFactory: async (cfg: ConfigService) => {
        const mongo = cfg.get('mongo') as any;
        ...
        const enableZstd = (process.env.MONGO_ENABLE_ZSTD ?? 'false') === 'true';
        const enableSnappy = (process.env.MONGO_ENABLE_SNAPPY ?? 'false') === 'true';

        if (enableZstd) {
          try { require.resolve('@mongodb-js/zstd'); compressors.push('zstd'); }
          catch { mongoLogger.warn('zstd requested but package not found'); }
        }
        if (enableSnappy) {
          try { require.resolve('@mongodb-js/snappy'); compressors.push('snappy'); }
          catch { mongoLogger.warn('snappy requested but package not found'); }
        }
