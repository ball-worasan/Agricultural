...
if (!is_file($databaseFile)) {
  app_log('profile_database_file_missing', ['file' => $databaseFile]);
...
if (!is_file($helpersFile)) {
  app_log('profile_helpers_file_missing', ['file' => $helpersFile]);
  http_response_code(500);
  echo '<div class="container"><h1>เกิดข้อผิดพลาด</h1><p>ไม่สามารถโหลดข้อมูลได้</p></div>';
  return;
}

$userServiceFile = APP_PATH . '/includes/UserService.php';
$imageServiceFile = APP_PATH . '/includes/ImageService.php';

require_once $databaseFile;
require_once $helpersFile;

if (is_file($userServiceFile)) {
  require_once $userServiceFile;
}
