...
if (!is_file($databaseFile)) {
  app_log('payment_database_file_missing', ['file' => $databaseFile]);
...
if (!is_file($helpersFile)) {
  app_log('payment_helpers_file_missing', ['file' => $helpersFile]);
  http_response_code(500);
  if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    json_response(['success' => false, 'message' => 'System error'], 500);
  } else {
...
if (!is_file($notificationServiceFile)) {
  app_log('payment_notification_service_missing', ['file' => $notificationServiceFile]);
